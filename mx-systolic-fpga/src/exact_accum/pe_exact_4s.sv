module mxfp8_mac_pe #(
    ...
    // Stage 2: Unsigned Product Computation
    logic [bit_width-1:0] data_right_stage2, data_bottom_stage2;
    logic valid_right_stage2, valid_bottom_stage2;
    logic unsigned [fi_prd_width-1:0] u_prd_stage2;
    logic prd_sign_stage2;
    logic [exp_width-1:0] op0_exp_stage2, op1_exp_stage2;
    logic op0_nrm_stage2, op1_nrm_stage2;

    // Stage 3: Product Sign Application and Shifting
    logic [bit_width-1:0] data_right_stage3, data_bottom_stage3;
    logic valid_right_stage3, valid_bottom_stage3;
    logic signed [fi_prd_width-1:0] prd_fi_stage3;
    logic signed [prd_width-1:0] prd_shifted_stage3;
    
    // Stage 4: Kulisch Accumulation
    logic [bit_width-1:0] data_right_stage4, data_bottom_stage4;
    logic valid_right_stage4, valid_bottom_stage4;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            // Calculate sign for next stage
            prd_sign_stage2 <= (op0_signed_man < 0) ^ (op1_signed_man < 0);
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            u_prd_stage2 <= u_op0 * u_op1;

            // Forward control signals and data
            ...
            op0_exp_stage2 <= op0_exp_stage1;
            op1_exp_stage2 <= op1_exp_stage1;
            op0_nrm_stage2 <= op0_nrm_stage1;
            op1_nrm_stage2 <= op1_nrm_stage1;
        end
    end
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            prd_fi_stage3 <= '0;
            prd_shifted_stage3 <= '0;
            valid_right_stage3 <= '0;
            valid_bottom_stage3 <= '0;
        end else begin
            // Apply sign to product
            prd_fi_stage3 <= prd_sign_stage2 ? -u_prd_stage2 : u_prd_stage2;

            // Compute shifted product
            prd_shifted_stage3 <= prd_fi_stage3 << ($unsigned({1'b0, op0_exp_stage2}) + 
                                                   $unsigned({1'b0, op1_exp_stage2}) - 
                                                   $unsigned(op0_nrm_stage2) - 
                                                   $unsigned(op1_nrm_stage2));
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            acc_reg <= '0;
            data_right_stage4 <= '0;
            data_bottom_stage4 <= '0;
            valid_right_stage4 <= '0;
            valid_bottom_stage4 <= '0;
        end else begin
            // Full-width accumulation with sign extension
            if (valid_right_stage3 && valid_bottom_stage3) begin
                acc_reg <= acc_reg + {{(out_width-prd_width){prd_shifted_stage3[prd_width-1]}}, 
                                    prd_shifted_stage3};
            end

            // Forward path
            data_right_stage4 <= data_right_stage3;
            data_bottom_stage4 <= data_bottom_stage3;
            valid_right_stage4 <= valid_right_stage3;
            valid_bottom_stage4 <= valid_bottom_stage3;
        end
    end

    // Output assignments
    assign data_out_right = data_right_stage4;
    assign data_out_bottom = data_bottom_stage4;
    assign valid_out_right = valid_right_stage4;
    assign valid_out_bottom = valid_bottom_stage4;
