...
module sr_adder_fp12 #(
    ...
    localparam FP12_SIG_W  = 6,
    localparam EXTRA       = CW - FP12_MANT_W,
    localparam TAIL_W      = 11,
    localparam REG_W       = (CW + 1) + TAIL_W,
    localparam SH_W        = $clog2(REG_W + 1),
    ...
    always_comb begin
        a_full_c = mant_a << EXTRA;
        a_sig_c  = (exp_a != 0) ? {1'b1, a_full_c} : '0;
        a_exp_c  = (exp_a != 0) ? $signed({2'b0, exp_a}) : '0;
        b_sig_c  = {1'b1, frac_b};
        b_exp_c  = $signed({2'b0, exp_b});

        a_ge_b_c = (a_exp_c > b_exp_c) || ((a_exp_c == b_exp_c) && (a_sig_c >= b_sig_c));
        ...
        shift_amt_c  = (shift_diff_c > REG_W) ? REG_W[SH_W-1:0] : shift_diff_c[SH_W-1:0];
        op_sub_c     = (x_sign_c != y_sign_c);
    end
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            close_path_s4 <= op_sub_c && (shift_amt_c <= 1);
    ...
    always_comb begin
        y_ext_c     = {y_full_s4, {TAIL_W{1'b0}}};
        y_shifted_c = y_ext_c >> shift_amt_s4;
    end
    ...
    always_comb begin
        y_shifted_final_c = op_sub_s4_q ? ((~y_shifted_c) + 1'b1) : y_shifted_c;
    end
    ...
    always_comb begin
        win_c = main_sum_s6[CW:0];
        zero_result_c = (win_c == '0);
        lz_c = 0;
        for (int b = CW; b >= 0; b--) begin
            if (win_c[b]) begin
                lz_c = CW - b;
                break;
            end
        end
        close_norm_sig_c = win_c << lz_c;
        close_norm_exp_c = x_exp_s6 - $signed({{(EXP_W-$bits(lz_c)){1'b0}}, lz_c});

        carry_out_c = main_sum_s6[CW+1];
        if (carry_out_c) begin
            far_norm_sig_c = main_sum_s6[CW+1:1];
            far_norm_exp_c = x_exp_s6 + 1;
            far_sel_s_c    = s2_s6;
        end else begin
            far_norm_sig_c = main_sum_s6[CW:0];
            far_norm_exp_c = x_exp_s6;
            far_sel_s_c    = s1_s6;
        end
    end
    ...
    always_comb begin
        ...
        // extra_bits_c below.
        native_top_c = corrected_s8[CW -: FP12_SIG_W];
        if (EXTRA > 0) begin
            extra_bits_c = corrected_s8[EXTRA_W-1:0];
            spare_rand_c = pass_s8.rand13[12 -: EXTRA_W];
        end else begin
            extra_bits_c = '0;
            spare_rand_c = '0;
        end
        round_sum_c = {1'b0, extra_bits_c} + {1'b0, spare_rand_c};
        carry_fin_c = (EXTRA > 0) ? round_sum_c[EXTRA_W] : 1'b0;
        native_sig_c = {1'b0, native_top_c} + carry_fin_c;

        if (native_sig_c[FP12_SIG_W]) begin
            final_exp_c  = corrected_exp_s8 + 1;
            mant_final_c = native_sig_c[FP12_SIG_W:1][FP12_MANT_W-1:0];
        end else begin
            final_exp_c  = corrected_exp_s8;
            mant_final_c = native_sig_c[FP12_MANT_W-1:0];
        end
    end
