...
//    logic [out_width-1:0] result [N*N];
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            for (int i = 0; i < N; i++) begin
                ...
                data_valid_west[i] <= (cycle_count >= 3*i) && 
                                    (cycle_count < k + 3*i);
                
                // North input valid signal (column-wise delay)
                data_valid_north[i] <= (cycle_count >= 3*i) && 
                                     (cycle_count < k + 3*i);
            end
        end
    end
    ...
    task feed_new_data(input int cycle);
        for (int i = 0; i < N; i++) begin
            ...
            if (cycle >= 3*i && cycle < k + 3*i) begin
                data_in_west[i] = test_data_west[i][cycle - 3*i];
            end else begin
                data_in_west[i] = '0;
            end
            ...
            if (cycle >= 3*i && cycle < k + 3*i) begin
                data_in_north[i] = test_data_north[i][cycle - 3*i];
            end else begin
                data_in_north[i] = '0;
            end
        end
